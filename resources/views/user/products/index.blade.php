...
@section('content')
    <!--Pageheader start-->
    <section class="bg-light d-flex flex-column align-items-center justify-content-center"
        style=" background: linear-gradient(45deg, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.22)), url({{ asset('user/images/jpg/page-header-img.jpg') }}) no-repeat;
                        ...
                        <div class="offcanvas-body ps-lg-2 pt-lg-0">
                            ...
                            <div class="mb-4">
                                ...
                                <ul class="list-unstyled mb-0 border-bottom">
                                    @foreach ($brands as $b)
                                        <li class="mb-2">
                                            <a href="{{ isset($category) ? route('product.category.brand', [$category->slug, $b->slug]) : route('product.brand', $b->slug) }}"
                                                class="text-decoration-none {{ isset($brand) && $brand->id == $b->id ? 'fw-bold text-primary' : 'text-dark' }}">
                                                {{ $b->title }}
                                            </a>
                                        </li>
                                    @endforeach
                                ...
                                <div class="collapse show" id="collapseColor" style="height: 180px" data-simplebar>
                                    @foreach ($colors as $color)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input filter-checkbox color-filter" type="checkbox"
                                                name="colors[]" id="color{{ $color->id }}" value="{{ $color->slug }}"
                                                {{ in_array($color->slug, explode('-', request('colors'))) ? 'checked' : '' }}>

                                            <label class="form-check-label" for="color{{ $color->id }}">
                                                <span class="d-inline-block rounded-circle me-2 border-black"
                                                    style="width: 16px; height: 16px; {{ $color->title == 'White' ? 'border: 2px solid #233122;' : '' }} background-color: {{ $color->hex_code }}"></span>
                                                {{ $color->title }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <div class="mb-4">
                                ...
                                <div>
                                    @for ($i = 5; $i >= 1; $i--)
                                        <div class="form-check mb-2">
                                            <input class="form-check-input filter-radio" type="radio" name="rating"
                                                id="rating{{ $i }}" value="{{ $i }}"
                                ...
                                <div class="d-flex gap-2 flex-wrap align-items-center" id="active-filters">
                                    ...
                                    @if (!empty($filters['colors'] ?? []))
                                        @foreach ($colors as $color)
                                            @if (in_array($color->slug, (array) $filters['colors']))
                                                <button type="button"
                                                    class="btn btn-sm btn-light d-flex align-items-center gap-2 filter-tag"
                                                    data-filter="color" data-value="{{ $color->slug }}">
                                                    {{ $color->title }}
                                    ...
                                    @if (request('price'))
                                        <button type="button"
                                            ...
                                            {{ isset(explode('-', request('price'))[1]) ? ' - ' . explode('-', request('price'))[1] : '' }}
                                            <i class="bi bi-x lh-1"></i>
                                        </button>
                                    @endif
                                    ...
                                    @if (request('colors') || request('price') || request('rating') || request('sort'))
                                        <button type="button" id="clearFilters" class="btn btn-sm btn-focus-none">
                                            Clear all
                                        </button>
                                    @endif
                                </div>
                                <div class="col-sm-4 col-xxl-3">
                                    <select class="form-select form-select-sm" id="sortSelect" name="sort">
                                        ...
                                        <option value="Best-selling"
                                            {{ request('sort') == 'Best-selling' ? 'selected' : '' }}>Best selling</option>
                                        <option value="Alphabetically-A-Z"
                                            {{ request('sort') == 'Alphabetically-A-Z' ? 'selected' : '' }}>
                                            Alphabetically,
                                            A-Z</option>
                                        <option value="Alphabetically-Z-A"
                                            {{ request('sort') == 'Alphabetically-Z-A' ? 'selected' : '' }}>
                                            Alphabetically,
                                            Z-A</option>
                                        ...
                                        <option value="Date-old-to-new"
                                            {{ request('sort') == 'Date-old-to-new' ? 'selected' : '' }}>
                                            Date, old to new
                                        </option>
                                        <option value="Date-new-to-old"
                                            {{ request('sort') == 'Date-new-to-old' ? 'selected' : '' }}>
                                            Date, new to old
                                        </option>
                                    </select>
                                </div>
