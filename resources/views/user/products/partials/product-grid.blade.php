    ...
    <div class="product-card">
        <div class="text-center product-card-img mb-4">
            <a href="{{ route('product.show', $product->slug) }}">
                <img src="{{ getFirstImageUrl($product) }}" alt="{{ $product->title }}" class="img-fluid" loading="lazy">
                <img src="{{ getImageUrl($product->media->skip(1)->first()) }}" alt="{{ $product->title }}" class="img-fluid product-img-hover" loading="lazy">
            </a>
            <div class="product-card-btn">
                <button type="button" class="btn btn-primary btn-icon btn-sm animate-pulse quick-view-btn" data-product-id="{{ $product->id }}" data-bs-toggle="modal" data-bs-target="#quickViewModal">
                    @include('user.svgs.quick-view-svg')
                </button>

                <button type="button" class="btn btn-primary btn-sm quick-add-btn" data-product-name="{{ $product->title }}" data-product-price="{{ $product->price }}" data-product-img="{{ getFirstImageUrl($product) }}">
        ...
        <div class="mb-4">
            ...
            <input type="radio" class="btn-check" name="colors{{ $product->id }}" id="color{{ $color->id }}" @checked($key==0) />
