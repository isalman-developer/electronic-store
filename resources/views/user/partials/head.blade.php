    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <!-- Favicon icon-->
    <link rel="apple-touch-icon" sizes="180x180" href="{{ asset('user/images/favicon/apple-touch-icon.png') }}" />
    <link rel="icon" type="image/png" sizes="32x32" href="{{ asset('user/images/favicon/favicon-32x32.png') }}" />
    <link rel="icon" type="image/png" sizes="16x16" href="{{ asset('user/images/favicon/favicon-16x16.png') }}" />
    <link rel="manifest" href="{{ asset('user/images/favicon/site.html') }}" />
    <link rel="shortcut icon" href="{{ asset('user/images/favicon/favicon.ico') }}" />

    @include('user.partials.styles')

    <title>@yield('title', 'Seco Official PK')</title>
